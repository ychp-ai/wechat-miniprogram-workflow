...
这是对话中的 Skill 路由约定，不是终端 CLI，也不依赖额外命令解析程序。识别用户消息中的子命令后，读取下表中对应的本地参考文件并执行。只加载被选中节点，不一次读取全部参考。
...
- 沿用项目框架、渲染器、后端和目录约定，不默认切换为原生、Skyline 或 CloudBase。新项目未指定时先明确这些关键选项。
- 交付物写入项目已有约定位置；无约定时文档放 docs/miniprogram/。只更新与本节点有关的内容，不覆盖用户已有结论。
- 结束时说明产物位置、实际验证结果、未完成项。建议后续节点可以，但不自行执行。
- 以下关联技能是按需增强项，不是必经工作流。使用时从当前可用技能目录定位并读取；遵守其真实依赖要求，不绕过缺失协议。若其依赖缺失，说明具体缺项并继续不依赖它的工作，不声称已执行该技能。不要远程抓取技能正文代替安装。

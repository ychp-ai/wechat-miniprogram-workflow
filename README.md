...
本工作流显式关联 16 个技能，按子命令和实际场景加载。完整名称、职责及间接依赖见 [关联技能清单](references/skill-links.md)，子命令映射见 [SKILL.md 的显式关联技能章节](SKILL.md#显式关联技能)。

清单区分直接关联与间接依赖；不会自动安装或加载全部技能。执行时说明实际使用项，缺失必需依赖时明确受阻操作。
